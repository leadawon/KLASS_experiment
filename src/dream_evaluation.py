...
from forward_stats import ForwardPassCounter, write_forward_stats
...
def test_dataset(
    ...
    if forward_counter is not None:
        stats = write_forward_stats(
            save_dir,
            model="Dream",
            dataset=dataset,
            total_items=total_items,
            num_samples=num_samples,
            total_forward_passes=forward_counter.total_forward_passes,
            extra={
                "alg": alg,
                "unmask_strategy": unmask_strategy,
            ...
            append_jsonl_path=stats_jsonl_path,
        )
        print(
            f"Forward passes: total={stats.total_forward_passes}, "
            f"avg/item={stats.avg_forward_passes_per_item:.4f}, "
            f"avg/sample={stats.avg_forward_passes_per_sample:.4f}"
        )
    ...
    if forward_counter is not None:
        stats = write_forward_stats(
            ...
            extra={
                ...
                "top_p": top_p,
                "k": list(k),
                "humaneval_workers": n_workers,
                "humaneval_timeout": timeout,
                "test_size": test_size,
                "random_sampling": random_sampling,
            },
            append_jsonl_path=stats_jsonl_path,
...
def main():
    ...
    forward_counter = ForwardPassCounter().attach(model)
    forward_counter.reset()
    stats_jsonl_path = os.path.join(args.save_dir, "forward_stats.jsonl")
